...
local function lock_group_friend(msg, data, target)
  ...
  if group_friend_lock == 'yes' then
    return 'friend setting is already locked'
  else
    data[tostring(target)]['settings']['friend'] = 'yes'
    save_data(_config.moderation.data, data)
...
local function unlock_group_friend(msg, data, target)
  ...
  local group_friend_lock = data[tostring(target)]['settings']['friend']
  if group_friend_lock == 'no' then
    return 'friend setting is not locked'
  else
    data[tostring(target)]['settings']['friend'] = 'no'
    save_data(_config.moderation.data, data)
...
local function lock_group_lock_ax(msg, data, target)
  ...
  if group_lock_ax_lock == 'yes' then
    return 'قفل بود'
  else
...
local function lock_group_lock_audio(msg, data, target)
  ...
  if group_lock_audio_lock == 'yes' then
    return 'قفل بود'
  else
...
local function lock_group_lock_video(msg, data, target)
  ...
  if group_lock_video_lock == 'yes' then
    return 'قفل بود'
  else
